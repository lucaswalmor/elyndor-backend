...
class MatchViewBuilder
{
    ...
    {
        $estado = $match->estado;
        $slot = $match->players()->where('user_id', $user->id)->value('player_slot');
        $opp = $slot === 1 ? 2 : 1;

        $players = [];
        foreach ([1, 2] as $s) {
            $p = $estado['jogadores'][(string) $s];
            $mp = $match->players()->where('player_slot', $s)->with('user')->first();
            $row = [
                'user_id' => $p['user_id'],
    ...
    private function hydrateField(array $units, bool $full = true): array
    {
        return array_map(function ($u) use ($full) {
            ...
            if ($full && $card) {
                $row['ataque'] = ($card->ataque + ($u['bonus_ataque'] ?? 0));
                $row['imagem_path'] = $card->imagem_path;
            }
